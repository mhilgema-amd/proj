...
# Default paths
#
PREFIX=${HOME}/martinh/app/openmpi
SRCDIR=${PREFIX}/src
WRKDIR=${PREFIX}/work
INSTALL_PREFIX=${HOME}/martinh/opt
...
function load_modules ()
{
    if which module; then
        module purge
        module load amd/rocm/${ROCM_VERSION}
        ...
        if [ -z "${C_INCLUDE_PATH}" ]; then
            export C_INCLUDE_PATH=${ROCM_PATH}/include
        else
            export C_INCLUDE_PATH=${ROCM_PATH}/include:${C_INCLUDE_PATH}
        fi
        if [ -z "${CPLUS_INCLUDE_PATH}" ]; then
            export CPLUS_INCLUDE_PATH=${ROCM_PATH}/include
        else
